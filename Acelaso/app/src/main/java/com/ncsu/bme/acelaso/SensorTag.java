...
public class SensorTag {
   ...
   /* public static float extractDieTempValues (BluetoothGattCharacteristic c, int rcv_active) {
        byte[]receive_bytes = c.getValue();
        Log.i(TAG, "Die Temp_Recieve:" + receive_bytes);
        DieBuffer = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN);
        if (DieBuffer.remaining() >= 4  && rcv_active == 2) {
            float dieTemp = DieBuffer.getFloat();*/
        /*if ((receive_bytes[0]!=(0x05)) | (receive_bytes[0]!=(0x06)) | (receive_bytes[0]!=(0x07)) |(receive_bytes[0]!=(0x08))) {
            float dieTemp = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN).getFloat();*/
           /* return dieTemp;
        }
        else return 0;
    }*/
    ...
    public static float extractGSRValues (BluetoothGattCharacteristic c, int rcv_active) {
        ...
        /*if ((receive_bytes[0]!=(0x05)) | (receive_bytes[0]!=(0x06)) | (receive_bytes[0]!=(0x07)) |(receive_bytes[0]!=(0x08))) {
            float gsr = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN).getFloat();*/
            return gsr;
        }
        else return 0;
    }
    ...
    public static float extractHRValues (BluetoothGattCharacteristic c, int rcv_active) {
        ...
        /*if ((receive_bytes[0]!=(0x05)) | (receive_bytes[0]!=(0x06)) | (receive_bytes[0]!=(0x07)) |(receive_bytes[0]!=(0x08))) {
            float hr = ByteBuffer.wrap(receive_bytes).order(ByteOrder.LITTLE_ENDIAN).getFloat();*/
            return hr;
        }
        else return 0;
    }
